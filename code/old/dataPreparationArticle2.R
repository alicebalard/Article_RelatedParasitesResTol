...
# 7 animals lost more than 18%, because they died overnight. 
# 5 were between 18 and 20% but got better
...
# For the 7, we will remove the last weight point, as it's the weight of a dehydrated cadaver
diedOvernight <- DSart2$EH_ID[DSart2$relWL >0.20 & !is.na(DSart2$relWL)]
DSart2$relWL[DSart2$EH_ID %in% diedOvernight & DSart2$relWL > 0.20 & !is.na(DSart2$relWL)] <- NA
...
ggplot(DSart2[DSart2$EH_ID %in% d2$EH_ID[1],], aes(dpi, relWL, group=EH_ID, col = EH_ID)) + 
  geom_point() + geom_line() 
